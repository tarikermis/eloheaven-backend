...
import { baseDomain, environment, stripePaypalSecretKey } from '@config';
import { safeFloat, safeInt } from '@helpers/number';
import { sha256 } from '@helpers/hash';

const paypal = new Stripe(stripePaypalSecretKey, {
  apiVersion: '2024-06-20',
  typescript: true,
});

export const createCustomerPaypal = async (payload: {
  name: string;
  email: string;
  phone?: string;
}) => {
  const customer = await paypal.customers.create({
    name: payload.name,
    email: payload.email,
    phone: payload.phone,
  });
...
export const createCheckoutSessionPaypal = async (payload: {
  product: {
    name: string;
    images: string[];
    description?: string;
  ...
  const product = await paypal.products.create(payload.product);
  ...
  const price = await paypal.prices.create({
