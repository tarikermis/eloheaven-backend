...
//? Row Templates
//? elo_boost = [ currentLeague , currentDivision , targetLeague  , targetDivision  , rrCurrent ]
//? irr_boost = [ rrRangeStart  , rrRangeEnd      , null          , null            , null      ]
    ...
    .keys({
      ...
      current: Joi.object()
        .keys({
          ...
          rr: Joi.number().integer().min(0).max(1000).required(),
        })
        .required(),
      target: Joi.object()
        .keys({
...
export default class Valorant_EloBoost extends Boost {
  // ELO BOOST
  private step; // sizeof Valorant_RRC
  private lastLeagueAt; // indexOf Immortal3

  // RR BOOST
  private minDiff; //rr boost min diff
  ...
  constructor(payload: IBoostPayload, parentSchema: Joi.ObjectSchema) {
    ...
    super(GameCode.Valorant, ServiceCode.Valorant_EloBoost, payload, extended, [
  ...
  public async process(): Promise<IOrder | ICalculateResult> {
    ...
    if (payload.checkout) {
      ...
      if (trr > 0) {
        orderTitle += `${capitalize(payload.general.target.tier)} ${trr} RR`;
      } else {
