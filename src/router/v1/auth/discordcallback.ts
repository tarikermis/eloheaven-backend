...
import { discordOauthAppId, discordOauthToken } from '@config';
  ...
  asyncHandler(async (req: IRoleRequest, res) => {
    try {
      ...
      const tokenResponse = await axios.post(
        ...
        new URLSearchParams({
          client_id: discordOauthAppId,
          client_secret: discordOauthToken,
      ...
      const discordUserProfile = userProfileResponse.data;

      const user = await UserRepo.findByEmail(discordUserProfile.email);

      if (!user) {
        const generatedUsername = discordUserProfile.email.split('@')[0];

        const { user: createdUser, keystore } = await UserRepo.create(
          {
            username: generatedUsername,
            email: discordUserProfile.email,
